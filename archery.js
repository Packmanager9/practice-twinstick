...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    function main() {
        ...
        if(player.health == 1){
            ...
            canvas_context.fillText("Dead", 350, 350)
        }
        start++
        if(start%rate == 0){
        let enemy = new Circle(Math.random()*1280, Math.random()*720, 20, "red")
        let link = new LineOP(player.body, enemy)
        ...
        for(let t =0;t<enemies.length;t++){
            if(enemies[t].doesPerimeterTouch(player.body)){
                player.health = 0
            }
            for(let k =0;k<enemies.length;k++){
                if(t!=k){
                    if(enemies[t].doesPerimeterTouch(enemies[k])){
                        let link = new LineOP(enemies[t], enemies[k])
                        enemies[t].xmom += (Math.cos(link.angle())*link.hypotenuse())/enemies[t].radius
                        enemies[k].xmom -= (Math.cos(link.angle())*link.hypotenuse())/enemies[t].radius
                        enemies[t].ymom += (Math.sin(link.angle())*link.hypotenuse())/enemies[t].radius
                        enemies[k].ymom -= (Math.sin(link.angle())*link.hypotenuse())/enemies[t].radius
                    }
                }
            }
        }
        for(let t =0;t<enemies.length;t++){
            ...
            let vector = new Vector(enemies[t], player.body.x-enemies[t].x, player.body.y-enemies[t].y)
            if( Math.abs(player.body.x-enemies[t].x) + Math.abs(player.body.y-enemies[t].y) > 1){
                vector.normalize(player.speed*.9)
                enemies[t].xmom = (enemies[t].xmom*.9) + (vector.xmom*.1)
                enemies[t].ymom = (enemies[t].ymom*.9) + (vector.ymom*.1)
            }
            enemies[t].move()
            enemies[t].draw()
        }
